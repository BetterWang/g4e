...
static char *GetTrueElementName(const char *name)
{
  ...
  }
  {
    ...
    if (!strcmp(rname+slen-2, "_6")  || !strcmp(rname+slen-2, "_S"))  rname[slen-2] = 0;
...
static bool IsElementStartLine(const char *name)
{
  ...
  return (!strcmp(name+slen-2, "_F") || !strcmp(name+slen-2, "_S")   || !strcmp(name+slen-3, "_E2") ||
	  !strcmp(name+slen-3, "_F\"") || !strcmp(name+slen-3, "_S\"") || !strcmp(name+slen-4, "_E2\""));
} // IsElementStartLine()
...
static bool IsElementEndLine(const char *name)
{
  ...
  return (!strcmp(name+slen-2, "_F") || !strcmp(name+slen-2, "_E")   || !strcmp(name+slen-3, "_E1") ||
	  !strcmp(name+slen-3, "_F\"") || !strcmp(name+slen-3, "_E\"") || !strcmp(name+slen-4, "_E1\"") || !strcmp(name+slen-3, "_6\""));
} // IsElementEndLine()
...
int main(int argc, char **argv)
{
  ...
  electron->AddDataFile("lepton.esr-ip6.tfs.txt");
  electron->AddSurveyFile("lepton.esr-survey-elke-ir6.tfs.txt");
  ...
  for(unsigned bl=0; bl<2; bl++) {
	...
	while (fgets(buffer, 1024, fin)) {
	  ...
	  if (vptr) {
		...
		printf("Orphan beam line element: %s\n", rname);
	      } //if
	    } //if
	  }
	} //while
    ...
    {
      char fname[1024];
      snprintf(fname, 1024-1, "%s/ir-magnets-2019-07-17-%s.dat", bl ? "E" : "H", bl ? "electrons" : "hadrons");
