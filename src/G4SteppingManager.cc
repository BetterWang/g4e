...
//---------------------------------
     ...
     /*  fsv 
     printf("G4SteppingManager.cc::  Volume=%s %s \n", fStep->GetPreStepPoint()->GetPhysicalVolume()->GetName().c_str(),fStep->GetPostStepPoint()->GetPhysicalVolume()->GetName().c_str());
     std::cout << "G4SteppingManager.cc:: Particle   " << fStep->GetTrack()->GetDynamicParticle()->GetDefinition()->GetParticleName() << std::endl ;
     std::cout << "xyz=  " << fStep->GetTrack()->GetPosition().x() << "  " << fStep->GetTrack()->GetPosition().y() << "  "  << fStep->GetTrack()->GetPosition().z() << std::endl;
     std::cout << "E=  " << fStep->GetTrack()->GetKineticEnergy() << std::endl;
     */
