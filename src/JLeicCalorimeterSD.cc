...
JLeicCalorimeterSD::JLeicCalorimeterSD(G4String name,
                                   JLeicDetectorConstruction* det)
:G4VSensitiveDetector(name),Detector(det)
{
  ...
  if (save_frames_root) {
    ...
    for (int in=0;in<12;in++) { 
      ...
      hmatrix1[in]->SetMinimum(0);  hmatrix1[in]->SetMaximum(25);
    }
...
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo....
void JLeicCalorimeterSD::hits_reset() {
...
G4bool JLeicCalorimeterSD::ProcessHits(G4Step* aStep,G4TouchableHistory*)
{
  ...
  G4int copyIDz_pre=0; if (use_depfet>0) copyIDz_pre  = touchablepre->GetCopyNumber(2);
  ...
  if (use_depfet>0) {
    ...
    // printf("VTX_ladder=%d \n",Mod);
    if (jDebug>2) printf("--> JLeicCalorimeterSD::ProcessHits() de=%f ADC=%d  len=%f Mod=%s  IDxy=(%d,%d,%d)\n",edep/keV,ADC,stepl/um,VTXmod.c_str(),copyIDx_pre,copyIDy_pre,copyIDz_pre);
    for(int in=0; in<12;in++) { 
  ...
  //-------------------------------------------------------------------------------------
  if (char_sh>0) {
    int iz=(225-zloc/um)/10;  if (iz>N_bin) iz=N_bin;  if (iz<0) iz=0;
    int ix = (xloc/um)/20.*10;   if (ix>5) ix=5;  if (ix<-5) ix=-5;
    int iy = (yloc/um)/20.*10;   if (iy>5) iy=5;  if (iy<-5) iy=-5;
  ...
  */
  ...
  //  printf("--> JLeicCalorimeterSD::ProcessHits() de=%f ADC=%d  len=%f  IDxy=(%d,%d) step(x,y,z)=(%f, %f %f) in=(%f, %f %f) um  out=(%f, %f %f) um \n  loc=(%f, %f %f) um part=%s\n"
   ...
   //--- save hits ------
  if (save_hits_root) {
    if (jDebug>6) printf("New Hit:: trkID=%d XYZloc (%f,%f,%f) dEdx=%f \n",aStep->GetTrack()->GetTrackID(),xloc,yloc,zloc,edep/keV); 

    int curTrackID = aStep->GetTrack()->GetTrackID();

    //----------------------------  fill Vectors -------------------------
    //-- fill hits --
    xpos.push_back(xstep/mm);
    ypos.push_back(ystep/mm);
    zpos.push_back(zstep/mm);
    dedx.push_back(edep/keV);
    ...
    //-- fill tracks --
    int texist=0; for (uint it=0; it<trkID.size(); it++) { if (trkID[it]==curTrackID) { texist=1; break; }  }
    if (texist==0) { //-- fill new track 
      if (jDebug>0) printf("++++> New Track::  trkID=%d PDG=%d  VTX=(%f,%f,%f)  Mom=(%f,%f,%f) \n"
			   ,curTrackID,PDG,vertex.x()/mm,vertex.y()/mm,vertex.z()/mm,vertexMom.x()*momentum.mag()/GeV,vertexMom.y()*momentum.mag()/GeV,vertexMom.z()*momentum.mag()/GeV);
      trkID.push_back(curTrackID);
      parID.push_back(ParrentID);
      trkPDG.push_back(PDG);
      xvtx.push_back(vertex.x()/mm);
      yvtx.push_back(vertex.y()/mm);
      zvtx.push_back(vertex.z()/mm);
      pxvtx.push_back(vertexMom.x()*momentum.mag()/GeV);
      pyvtx.push_back(vertexMom.y()*momentum.mag()/GeV);
      pzvtx.push_back(vertexMom.z()*momentum.mag()/GeV);
      trkMom.push_back(momentum.mag()/GeV);
      track_size++;

      printf("store Mom.x=%f \n",vertexMom.x()*momentum.mag()/GeV);

    } else {
      if (jDebug>6) printf("    > OLD Track:: trkID=%d  VTX=(%f,%f,%f) \n",curTrackID,vertex.x(),vertex.y(),vertex.z()); 
    }
  ...
  if (jDebug>2) printf("--> JLeicCalorimeterSD::ProcessHits() Vol=(%s) %p Abs=%p \n",physVol->GetName().c_str(),(void*)physVol,(void*)Detector->GetAbsorber());
...
void JLeicCalorimeterSD::EndOfEvent(G4HCofThisEvent* HCE)
{
  ...
  if (use_fdc)  {
    ...
    ntr=0; ntr1=0; ntr2=0; ntr3=0;
    for (int ii=0;  ii<100; ii++) if (dEslice[ii]>5) { ntr++; if (ii<5) ntr1++; if (ii<10) ntr2++; if (ii<15) ntr3++; }
 ...
 if (save_hits_root) { //--   fill tree  ----
    ...
    if(evt) event_num = evt->GetEventID();
...
//----------------------------- command line args ------------------------------
    ...
    strncpy(DATA_HOST,"localhost",LHOST);   DATA_PORT=20248;
	...
	for (i=0;i<nmod;i++) {
	  ...
	  if (itrg%prescale==0 || itrg<20) {
	    ...
	    printf("==> SEND:: Trg=%d(%d) Mod=%d(%d) siz=%d(%d), rate=%f\n"
		   ,evtTrigID,itrg,evtModID,i,evtSize,LENEVENT,rate);
