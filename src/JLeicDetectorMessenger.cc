...
//  }
  ...
  if( command == WorldMaterCmd )
   { JLeicDetector->SetWorldMaterial(newValue);}
   
  if( command == AbsThickCmd )
   { JLeicDetector->SetAbsorberThickness(AbsThickCmd->GetNewDoubleValue(newValue));}

  if( command == RadiatorThickCmd )
   { JLeicDetector->SetRadiatorThickness(RadiatorThickCmd->
                   GetNewDoubleValue(newValue));}

  if( command == GasGapThickCmd )
   { JLeicDetector->SetGasGapThickness(GasGapThickCmd->
                   GetNewDoubleValue(newValue));}
   
  if( command == AbsRadCmd )
   { JLeicDetector->SetAbsorberRadius(AbsRadCmd->GetNewDoubleValue(newValue));}
   
  if( command == AbsZposCmd )
   { JLeicDetector->SetAbsorberZpos(AbsZposCmd->GetNewDoubleValue(newValue));}
   
  if( command == WorldZCmd )
   { JLeicDetector->SetWorldSizeZ(WorldZCmd->GetNewDoubleValue(newValue));}
