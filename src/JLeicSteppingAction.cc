...
//   my_or= (aStep->GetTrack()->GetPosition().x()+0.3529979921)*(aStep->GetTrack()->GetPosition().x()+0.3529979921) 
        ...
        my_or = (aStep->GetTrack()->GetPosition().x() / 1000. + 0.3529979921) *
                (aStep->GetTrack()->GetPosition().x() / 1000. + 0.3529979921)
                + (200 * aStep->GetTrack()->GetMomentumDirection().x() + 200 * tan(0.05299800449)) *
                  (200 * aStep->GetTrack()->GetMomentumDirection().x() + 200 * tan(0.05299800449))
                + (aStep->GetTrack()->GetPosition().y() / 1000.) * (aStep->GetTrack()->GetPosition().y() / 1000.)
                + (200 * aStep->GetTrack()->GetMomentumDirection().y()) *
                  (200 * aStep->GetTrack()->GetMomentumDirection().y());

        printf(">> %f %f %f %f %f %f \n", aStep->GetTrack()->GetPosition().x(), aStep->GetTrack()->GetPosition().y(),
               aStep->GetTrack()->GetPosition().z(), aStep->GetTrack()->GetMomentumDirection().x(),
    ...
    */
    ...
    if (aStep->GetPreStepPoint()->GetPhysicalVolume()->GetName() == "pxdPixel" ||
        ...
        runaction->FillHist(7, zend - detector->fAbsorberZ + detector->fAbsorberThickness / 2.,
                            aStep->GetTotalEnergyDeposit() / keV);
        if (((aStep->GetTrack()->GetDynamicParticle()->GetDefinition()->
                GetParticleName()) == "gamma")) {
        ...
        if (((aStep->GetTrack()->GetDynamicParticle()->GetDefinition()->
            ...
            if ((aStep->GetTrack()->GetTrackID() == 1) && (aStep->GetTrack()->GetParentID() == 0)) {
                runaction->FillHist(5, zend - detector->fAbsorberZ + detector->fAbsorberThickness / 2.,
                                    aStep->GetTotalEnergyDeposit() / keV);
            } else {
                runaction->FillHist(4, zend - detector->fAbsorberZ + detector->fAbsorberThickness / 2.,
                                    aStep->GetTotalEnergyDeposit() / keV);
            }
            runaction->FillHist(6, zend - detector->fAbsorberZ + detector->fAbsorberThickness / 2.,
                                aStep->GetTotalEnergyDeposit() / keV);
        }
        if ((aStep->GetTrack()->GetDynamicParticle()->GetDefinition()->
                GetParticleName()) == "gamma") {
