...
public:
    ...
    inline void ConstructDetectors() {
        ...
        ConstructRadiator(); // needs to be constructed always!
        ConstructAbsorber();
        printf("fFoilNumber2=%d \n",cfg.fFoilNumber);

    };
    //------------------------------------------------------------------
    inline void ConstructAbsorber() {
        ...
        ci_TRD_det_Phys = new G4PVPlacement(0, G4ThreeVector(0, 0, cfg.det_PosZ), "ci_TRD_det_Physics", ci_TRD_det_Logic,
                                            Phys, false, 0);


    };
        inline void ConstructRadiator() {
            static char abname[256];
            auto cfg = ConstructionConfig;
