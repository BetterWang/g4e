...
#ifndef G4E_FFI_RPOT_HH
#define G4E_FFI_RPOT_HH
...
class ffi_RPOT_Design {
public:
    inline void Construct(ffi_RPOT_Config cfg, G4Material *worldMaterial, G4VPhysicalVolume *motherVolume) {
        printf("Begin ffi_RPOT volume \n");
    ...
    inline void ConstructDetectors() {
        printf("Begin ffi_RPOT detector volumes \n");
...
//===================================================================================
        ...
        for (int lay = 0; lay < cfg.Nlayers; lay++) {
            ...
            ffi_RPOT_lay_RIn[lay] = cfg.RIn + 1 * cm + (double(lay) * 0.5) * cm;
            ffi_RPOT_lay_ROut[lay] = cfg.ROut - 25 * cm + (double(lay) * 2.) * cm;;

            //      ffi_RPOT_lay_PosZ[lay]=-ffi_RPOT_GVol_PosZ/2+(double(lay)*5.)*cm;
            ffi_RPOT_lay_PosZ[lay] = -cfg.SizeZ / 2 + 5 * cm + (double(lay) * 3.) * cm;
            ffi_RPOT_lay_SizeZ[lay] = 1 * cm;

            sprintf(abname, "ffi_RPOT_lay_Solid_%d", lay);
            lay_Solid[lay] = new G4Tubs(abname, ffi_RPOT_lay_RIn[lay], ffi_RPOT_lay_ROut[lay],
                                        ffi_RPOT_lay_SizeZ[lay] / 2., 0., 360 * deg);

            sprintf(abname, "ffi_RPOT_lay_Logic_%d", lay);
            lay_Logic[lay] = new G4LogicalVolume(lay_Solid[lay],
                                                 ffi_RPOT_lay_Material, abname);

            attr_ffi_RPOT_lay = new G4VisAttributes(G4Color(0.8, 0.4, 0.3, 0.8));
            attr_ffi_RPOT_lay->SetLineWidth(1);
            attr_ffi_RPOT_lay->SetForceSolid(true);
            lay_Logic[lay]->SetVisAttributes(attr_ffi_RPOT_lay);

            sprintf(abname, "ffi_RPOT_lay_Phys_%d", lay);
    ...
    ffi_RPOT_Config  ConstructionConfig;
