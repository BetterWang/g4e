                                                                                         ...
                                                                                         fIonExciteEnergy(0.0), fIonFloatingLevelBase('\0'), fIonEnergyLevel(0)
{
    ...
    // set initial value to ConeParticleGunHW
...
void ConeParticleGunMessengerEta::SetNewValue(G4UIcommand *command, G4String newValues)
{
    ...
    else if (command == minMomCmd) { fParticleGun->SetParticleMinMomentum(minMomCmd->GetNewDoubleValue(newValues)); }
    else if (command == maxMomCmd) { fParticleGun->SetParticleMaxMomentum(maxMomCmd->GetNewDoubleValue(newValues)); }
    
    else if (command == minEtaCmd) { fParticleGun->SetParticleMinEta(minEtaCmd->GetNewDoubleValue(newValues)); }
    else if (command == maxEtaCmd) { fParticleGun->SetParticleMaxEta(maxEtaCmd->GetNewDoubleValue(newValues)); }
...
G4String ConeParticleGunMessengerEta::GetCurrentValue(G4UIcommand *command)
{
    ...
    if (command == directionCmd) { cv = directionCmd->ConvertToString(fParticleGun->GetParticleMomentumDirection()); }
        ...
        if (ene == 0.) { G4cerr << " ConeParticleGunHW:  was defined in terms of momentum." << G4endl; }
        ...
        if (mom == 0.) { G4cerr << " ConeParticleGunHW:  was defined in terms of kinetic energy." << G4endl; }
        else {
