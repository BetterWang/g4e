...
namespace g4e {
    ...
    public:
        ...
        void Parse() {
            ...
            this->pzlep = std::stod(
                    event_tokens[6]);    // D lab pz of the lepton beam (sign(pz)*p for non-zero crossing angle)
            this->pztarg = std::stod(
                    event_tokens[7]);    // D lab pz/A of the ion beam (sign(pz)*p/A for non-zero crossing angle)
            this->pznucl = std::stod(
                    event_tokens[8]);    // D lab pz of the struck nucleon (sign(pz)*p for non-zero crossing angle)
            this->crang = std::stod(
                    event_tokens[9]);    // D crossing-angle (mr). crang = (1000*) atan(px/pz) for the beam momentum with nonzero px. Note: we assume one of the beams has px=py=0 and the other py=0.
            this->crori = std::stoi(
                    event_tokens[10]);    // 10 I  crossing angle orientation. lepton beam defines \pm\hat{z} direction. \pm 2 hadron/ion beam defines
            //    \pm\hat{z} direction. 0 means no crossing angle.
            this->subprocess = std::stoi(
                    event_tokens[11]);    //11 I   pythia subprocess (MSTI(1)), for details see PYTHIA
            ...
            this->beamparton = std::stoi(
                    event_tokens[15]);    //15 I  in case of resolved photon processes and soft VMD the virtual photon has a hadronic structure. This gives the info which parton interacted with the target parton (MSTI(15))
            ...
            this->leptonphi = std::stod(
                    event_tokens[23]);    //23 D  phi of the lepton (VINT(313)) in the laboratory frame
            ...
            this->F2 = std::stod(
                    event_tokens[29]);    //29 D  information used and needed in the radiative correction code
            this->F1 = std::stod(
                    event_tokens[30]);    //30 D  information used and needed in the radiative correction code
            this->R = std::stod(
                    event_tokens[31]);    //31 D  information used and needed in the radiative correction code
            this->sigma_rad = std::stod(
                    event_tokens[32]);    //32 D  information used and needed in the radiative correction code
            this->SigRadCor = std::stod(
                    event_tokens[33]);    //33 D  information used and needed in the radiative correction code
            this->EBrems = std::stod(
                    event_tokens[34]);    //34 D  energy of the radiative photon in the nuclear rest frame
            this->photonflux = std::stod(event_tokens[35]);    //35 D  flux factor from PYTHIA (VINT(319))
            this->b = std::stod(
                    event_tokens[36]);    //36 D  Impact parameter (in fm), radial position of virtual photon w.r.t. the center of the nucleus in the nuclear TRF with z along γ*
            this->Phib = std::stod(
                    event_tokens[37]);    //37 D  azimuthal angle of the impact parameter of the virtual photon in the nuclear TRF with z along γ* and φ=0 for the scattered electron
            ...
            this->Ncollt = std::stoi(
                    event_tokens[40]);    //40 I  Number of collisions between the incoming γ* and nucleons in the nucleus (same as number of participating nucleons)
            this->Ncolli = std::stoi(
                    event_tokens[41]);    //41 I  Number of inelastic collisions between the incoming γ* and nucleons in the nucleus (same as number of inelastically participating nucleons)
            this->Nwound = std::stoi(
                    event_tokens[42]);    //42 I  Number of wounded nucleons, including those involved in the intranuclear cascade. WARNING: This variable is not finalized yet
            this->Nwdch = std::stoi(
                    event_tokens[43]);    //43 I  Number of wounded protons, including those involved in the intranuclear cascade. WARNING: This variable is not finalized yet
            ...
            this->Nincch = std::stoi(
                    event_tokens[48]);    //48 I  Number of charged stable hadrons from the Intra Nuclear Cascade
            this->d1st = std::stod(
                    event_tokens[49]);    //49 D  density-weighted distance from first collision to the edge of the nucleus (amount of material traversed / ρ0)
            this->davg = std::stod(
                    event_tokens[50]);    //50 D  Average density-weighted distance from all inelastic collisions to the edge of the nucleus
            this->pxf = std::stod(
                    event_tokens[51]);    //51 D  Fermi-momentum of the struck nucleon (or sum Fermi momentum for all inelastic nucleon participants for Ncolli>1) in target rest frame with z along gamma* direction
            this->pyf = std::stod(
                    event_tokens[52]);    //52 D  Fermi-momentum of the struck nucleon (or sum Fermi momentum for all inelastic nucleon participants for Ncolli>1) in target rest frame with z along gamma* direction
            this->pzf = std::stod(
                    event_tokens[53]);    //53 D  Fermi-momentum of the struck nucleon (or sum Fermi momentum for all inelastic nucleon participants for Ncolli>1) in target rest frame with z along gamma* direction
            this->Eexc = std::stod(
                    event_tokens[54]);    //54 D  Excitation energy in the nuclear remnant before evaporation and breakup.
            this->RA = std::stod(
                    event_tokens[55]);    //55 D  Nuclear PDF ratio for the up sea for the given event kinematics (x,Q2), but set to 1 if multi-nucleon shadowing is off (genShd=1)
            ...
            this->nrTracks = std::stoi(
                    event_tokens[59]);    //59 I  number of tracks in this event, including event history

            this->is_parsed = true;
        }

        virtual ~BeagleEventData() = default;
        ...
        std::vector<std::unique_ptr<g4e::BeagleParticle>> particles; // Primary particles (that comes from a generator)
