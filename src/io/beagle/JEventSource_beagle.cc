    ...
    services(app)
{
    ...
    text_reader = std::unique_ptr<ej::TextEventFileReader>(new TextEventFileReader(
            source_name,
            [](uint64_t line_num, std::string line) {
            ...
            [](uint64_t, std::vector<std::string> tokens){
    ...
    mFactoryGenerator = new JSourceFactoryGenerator<BeagleParticle>();
...
// Destructor
//----------------
JEventSource_beagle::~JEventSource_beagle()
{
...
{
    // Read an event from the source and copy the vital info into the GeantEvent structure.
