...
#include "ReferenceDetectorTrackingAction.hh"
...
int ReferenceDetectorTrackingAction::gTrackSaveSecondaryLevel = -1;

ReferenceDetectorTrackingAction::ReferenceDetectorTrackingAction(): G4UserTrackingAction()
{
    ...
    mTrackSaveSecondaryLevel = gTrackSaveSecondaryLevel;
    G4cout << "/g4e/tracking/saveSecondaryLevel = " << mTrackSaveSecondaryLevel << G4endl;
}

void ReferenceDetectorTrackingAction::PreUserTrackingAction(const G4Track *aTrack)
{
    CommonTrackInformation * info;

    

    if(aTrack->GetParentID() == 0) {
        info = new CommonTrackInformation();
        info->SetLevel(0);

        aTrack->SetUserInformation(info);
    } else {
        info = (CommonTrackInformation*) aTrack->GetUserInformation();
...
void ReferenceDetectorTrackingAction::PostUserTrackingAction(const G4Track *aTrack)
{
    auto info = (CommonTrackInformation*) aTrack->GetUserInformation();
    ...
    for(auto secTrack: *secTracks) {
        auto secInfo = new CommonTrackInformation();
