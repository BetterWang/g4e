...
JLeicXTRphysicsMessenger::JLeicXTRphysicsMessenger(JLeicXTRphysics* List)
...
void JLeicXTRphysicsMessenger::SetNewValue(G4UIcommand* command,G4String newValue)
{
  ...
  if(command == cutGCmd)
    { JLeicList->SetGammaCut(cutGCmd->GetNewDoubleValue(newValue));}
  if(command == cutECmd)
    { JLeicList->SetElectronCut(eCmd->GetNewDoubleValue(newValue));}
  if(command == setMaxStepCmd)
    { JLeicList->SetMaxStep(setMaxStepCmd->GetNewDoubleValue(newValue));}
  ...
  {
    printf("  --> JLeicXTRphysicsMessenger::SetNewValue SetRegGammaCut =%s !!!\n",newValue.c_str());
