...
//  new G4PVPlacement(0, G4ThreeVector(0,0,ZPosVect*cm), lMagField, "Magnet",World_Logic, false, 0);
    ...
    void CreateQuad(G4VPhysicalVolume *fMotherPhysVolume)
    {
        ...
        printf("CreateQuad:: name=%s  FIELD = Dx %f  Dy %f --  Qn %f Qs %f -- Sek %f  Sol %f \n", name.c_str(), DipoleFieldBx, DipoleFieldBy, QuadrupolFieldQnorm,
               QuadrupolFieldQskew, SextupoleField, SolenoidField);
...
public:
    ...
    //-----------------------------------------------------------
    void parse_line(std::string line, int _USE_LINE)
    {
        USE_LINE = _USE_LINE;
        ...
        QMag *qmag = new QMag(name, type, LengthZ, Rin, Rin2, Rout, DipoleFieldBx, DipoleFieldBy, QuadrupolFieldQnorm, QuadrupolFieldQskew, SextupoleField, SolenoidField, Xcenter,
                              Ycenter, Zcenter, MagTheta, MagPhi, fMotherPhysVolume, _USE_LINE);
