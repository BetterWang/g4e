...
class ce_EMCAL_DigiHit
{
public:

    ce_EMCAL_DigiHit();

    ce_EMCAL_DigiHit(const ce_EMCAL_DigiHit &);

    ce_EMCAL_DigiHit(int);

    ce_EMCAL_DigiHit*  CreateHit(ce_EMCAL_Hit *aHit);

    ~ce_EMCAL_DigiHit() ;//override;
    ...
    const ce_EMCAL_DigiHit &operator=(const ce_EMCAL_DigiHit &);
...
extern G4ThreadLocal G4Allocator<ce_EMCAL_DigiHit> *JLeicCe_emcalDigiHitAllocator;


inline void *ce_EMCAL_DigiHit::operator new(size_t)
{
    void *aHit;
    if (!JLeicCe_emcalDigiHitAllocator) JLeicCe_emcalDigiHitAllocator = new G4Allocator<ce_EMCAL_DigiHit>();
    aHit = (void *) JLeicCe_emcalDigiHitAllocator->MallocSingle();
    return aHit;
}


inline void ce_EMCAL_DigiHit::operator delete(void *aHit)
{
    if (!JLeicCe_emcalDigiHitAllocator) JLeicCe_emcalDigiHitAllocator = new G4Allocator<ce_EMCAL_DigiHit>();
    JLeicCe_emcalDigiHitAllocator->FreeSingle((ce_EMCAL_DigiHit *) aHit);
}
