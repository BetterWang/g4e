...
#ifndef G4E_FFI_OFFM_TRK_HH
#define G4E_FFI_OFFM_TRK_HH
...
class ffi_OFFM_TRK_Design {
public:
    inline void Construct(ffi_OFFM_TRK_Config cfg, G4Material *worldMaterial, G4VPhysicalVolume *motherVolume) {
        printf("Begin ffi_OFFM_TRK volume \n");

        ConstructionConfig = cfg;
        // create  a global volume for your detectors

	//       Solid = new G4Tubs("ffi_OFFM_TRK_GVol_Solid",cfg.RIn, cfg.ROut,
	//                  cfg.SizeZ / 2., 0., 360 * deg);
       Solid = new G4Box("ffi_OFFM_TRK_GVol_Solid",cfg.SizeX /2., cfg.SizeY/2., cfg.SizeZ / 2.);

 
        Logic = new G4LogicalVolume(Solid, worldMaterial, "ffi_OFFM_TRK_GVol_Logic");

        // ci_GEM_GVol_PosZ= SizeZ/2-abs(World_ShiftVTX)+ci_GEM_GVol_SizeZ-5*cm;   // --- need to find out why this 5 cm are needed
        Phys = new G4PVPlacement(0, G4ThreeVector(cfg.Xpos, 0, cfg.Zpos), "ffi_OFFM_TRK_GVol_Phys", Logic,
	...
	//       lay_Solid = new G4Tubs("ffi_OFFM_TRK_lay_Solid", ffi_OFFM_TRK_lay_RIn, ffi_OFFM_TRK_lay_ROut,
        ...
        lay_Solid = new G4Box("ffi_OFFM_TRK_lay_Solid",cfg.SizeX /2.-1*mm, cfg.SizeY/2.-1*mm ,
                                      ffi_OFFM_TRK_lay_SizeZ / 2.);
        lay_Logic = new G4LogicalVolume(lay_Solid, ffi_OFFM_TRK_lay_Material, "ffi_OFFM_TRK_lay_Logic");
  ...
  //    G4Tubs *Solid;      //pointer to the solid
    ...
    ffi_OFFM_TRK_Config  ConstructionConfig;
