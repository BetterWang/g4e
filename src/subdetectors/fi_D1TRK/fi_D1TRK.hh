...
public:
    inline void ConstructA(fi_D1TRK_Config cfg, G4Material *worldMaterial, G4VPhysicalVolume *motherVolume)
    {
        ...
        Solid = new G4Tubs("fi_D1TRK_GVol_Solid", cfg.RIn, cfg.ROut, cfg.SizeZ / 2., 0., 360 * deg);
        ...
        Phys = new G4PVPlacement(0, G4ThreeVector(0, 0, cfg.Zpos), "fi_D1TRK_GVol_Phys", Logic, motherVolume, false, 0);
    ...
    inline void ConstructDetectorsA()
    {
        ...
        for (int lay = 0, ffsi_counter = 0; lay < cfg.Nlayers; lay++, ffsi_counter++) {
            ...
            // Create SOLID
            auto solidName = fmt::format("fi_D1_TRK_lay_Solid_{}", lay);
            f1_D1_lay_Solid[lay] = new G4Tubs(solidName, fi_D1_lay_RIn, fi_D1_lay_ROut, fi_D1_lay_SizeZ / 2., cfg.PhiStart, cfg.PhiTot);

            // Create logic volume
            // for JLEIC        f1_D1_lay_Solid = new G4Tubs("f1_D1_lay_Solid", fi_D1_lay_RIn, fi_D1_lay_ROut,
            //                                    fi_D1_lay_SizeZ / 2., 170., 330 * deg);
            auto logicName = fmt::format("fi_D1_TRK_lay_Logic_{}", lay);
            f1_D1_Lay_Logic[lay] = new G4LogicalVolume(f1_D1_lay_Solid[lay], fi_D1_lay_Material, logicName);

            // Create physical volume
            double Z = -cfg.SizeZ / 2 + (lay + 1) * fi_D1_lay_SizeZ / 2 + (lay + 1) * 5 * cm;
            auto physName = fmt::format("fi_D1_TRK_lay_Phys_{}", lay);
            f1_D1_lay_Phys[lay] = new G4PVPlacement(nullptr, G4ThreeVector(0, 0, Z), physName, f1_D1_Lay_Logic[lay], Phys, false, ffsi_counter);
